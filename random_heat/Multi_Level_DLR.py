...
#                 func += interpolate(self.U_n[k][j],self.V[level]) * Constant(self.Y_n[k][j][i])
#         return func
...
#             ans += (a[i]* grad_list[i]* Y[i])
#         ans /= self.sample_size
#         ans = project(ans,VectorFunctionSpace(self.mesh[level], 'P', 1))
#         if level == 0:    
#             return ans
#         else:
#             grad_list2 = [grad(self._build_function(i,level-1)) for i in range(self.sample_size)]
#             ans2 = a[0] * grad_list2[0] * Y[0]
#             for i in range(1, self.sample_size):
#                 ans2 += a[i] * grad_list2[i] * Y[i]
#             ans2 /= self.sample_size
#             ans2 = project(project(ans2, VectorFunctionSpace(self.mesh[level-1], 'P', 1)),VectorFunctionSpace(self.mesh[level], 'P', 1))
#             return ans - ans2
...
#             for i in range(self.R[level]):
#                 for j in range(self.sample_size):
#                     ans[i][j] = assemble(dot(projet(a[j] * grad(u[j]),VectorFunctionSpace(self.mesh[level], 'P', 1)),grad(self.U[level][i])) * dx)        
#             return ans
#         else:
#             u2 = [self._build_function(i,level-1) for i in range(self.sample_size)]
#             for i in range(self.R[level]):
#                 for j in range(self.sample_size):
#                     ans[i][j] = assemble(dot(projet(a[j] * grad(u[j]),VectorFunctionSpace(self.mesh[level], 'P', 1)) - project(projet(a[j] *grad(u2[j]),VectorFunctionSpace(self.mesh[level-1], 'P', 1)),VectorFunctionSpace(self.mesh[level], 'P', 1)) ,grad(self.U[level][i])) * dx)        
#             return ans
